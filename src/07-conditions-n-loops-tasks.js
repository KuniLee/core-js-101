...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    for (let j = 0; j < str.length; j++) {
      ...
      if (str[i] !== str[j] && j === str.length - 1) return str[i];
    }
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  ...
  ccn = ccn.toString().split('').map((el) => +el);
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((accu, n) => accu + +n, 0));
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i++) {
    ...
    if (pathes.every((el) => el[i] === char)) comPath += char; else break;
  }
  return comPath.replace(/(?<=\/)\w+(?=$)/, '');
}
